...
feature
	...
	main_soc: detachable NETWORK_STREAM_SOCKET
...
feature
	...
	connect_to_server(server_ip_address: STRING server_port: INTEGER my_local_port: INTEGER): BOOLEAN
		...
		do
			create main_soc.make_client_by_port (server_port, server_ip_address)
			create addr.make_any_local (my_local_port)

			if
				attached main_soc as soc
			then
	...
	cleanup_connection
		do
			if attached main_soc as soc then
                soc.cleanup
                soc.dispose
            end
		end
